...
class Backend(PeriodicImportBackend):
    ...
    _general_description = {
        GenericBackend.BACKEND_NAME: "backend_trello",
        GenericBackend.BACKEND_HUMAN_NAME: _("Trello"),
        GenericBackend.BACKEND_AUTHORS: ["Frédéric Descamps"],
        GenericBackend.BACKEND_TYPE: GenericBackend.TYPE_READWRITE,
        GenericBackend.BACKEND_DESCRIPTION:
        _("This service synchronizes your tasks with Trello\n\n"
          "Note: This product uses Trello's API."
          "How cool is that !"),
    }
    ...
    _static_parameters = {
        "api_key": {
            ...
            GenericBackend.PARAM_DEFAULT_VALUE:
                '4da6ce8dae245e23bb9f09f19556e212', },
        "oauth_token": {
            GenericBackend.PARAM_TYPE: GenericBackend.TYPE_STRING,
            GenericBackend.PARAM_DEFAULT_VALUE:
                'user@example.com', },
    ...
    def __init__(self, params):
        ...
        self.data_path = os.path.join('backends/trello/',
    ...
    def initialize(self):
        ...
        self.hash_dict_path = os.path.join('backends/trello/',
                                      "hash_dict-" + self.get_id() + \
                                      self._parameters["oauth_token"])
        #print "Data path = \n****\n****\n" + str(self.hash_dict_path) + "\n****\n****\n"
        self.hash_dict = self._load_pickled_file(self.hash_dict_path, \
                                                 default_value = {})
        
        pynotify.init("started")
        self._client = Trello(self._parameters["api_key"], self._parameters["oauth_token"], self._parameters["board_id"])
        #print "returned here"
            
    def do_periodic_import(self, ):
        #print "Importing ..."
        lists = self._client.getLists()
        print lists
        cards = []
        # we need to go thru all the list that will be considered as tag
        for listname in lists:
            #tags.append(listname['name'])
            list = self._client.findList(listname['name'])
            for card in self._client.getCards(list['id']):
                cards.append(card)
    ...
    def fetch_tasks_from_server(self, ):
        ...
        params = {"email": self._parameters["oauth_token"],
                  "password": self._parameters["board_id"],}

        tasks = requests.post("%s%s" % (self._parameters["api_key"], self.URLS['tasks']['get']), params)
    ...
    def remote_add_tasks(self, task_list):
        ...
        params = {
            "email": self._parameters["oauth_token"],
            "password": self._parameters["board_id"],
            "task_list": json.dumps(task_list),
        }
        ids = requests.post("%s%s" % (self._parameters["api_key"], self.URLS['tasks']['new']), \
    ...
    def remote_update_task(self, task, task_id):
        ...
        response = requests.post("%s%s" % (self._parameters["api_key"], self.URLS['tasks']['update']), params)
    ...
    def remote_delete_task(self, web_id_list):
        ...
        tags = requests.post("%s%s" % (self._parameters["api_key"], self.URLS['tasks']['delete']), params)
    
    def fetch_tags_from_server(self, ):
        #print "Fetching tags started ..."
        params = {"email": self._parameters["oauth_token"],
                  "password": self._parameters["board_id"],}
        tags = requests.post("%s%s" % (self._parameters["api_key"], self.URLS['tags']), params)
